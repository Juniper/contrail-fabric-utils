...
def purge_node_from_rabbitmq_cluster(del_rabbitmq_node, role):
    ...
    if ping_test(del_rabbitmq_node):  
        ...
        with settings(host_string = env.roledefs[0], warn_only = True):
            hostname = local('getent hosts %s | awk \'{print $3\'}' % del_rabbitmq_ip, capture = True)
            local("rabbitmqctl forget_cluster_node rabbit@%s" % hostname)

    # Giving some time for the other nodes to assume mastership, 
    # if required
    time.sleep(30)
