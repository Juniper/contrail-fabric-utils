...
def verfiy_and_update_hosts(host_name, host_string):
        ...
        run("echo '%s          %s          %s' >> /etc/hosts" % (host_string.split('@')[1], host_name, host_name+ctrl))
...
def rabbitmq_env():
    ...
    rabbitmq_env_conf = rabbitmq_env_conf_template.template.safe_substitute({
    ...
    cfg_file.write(rabbitmq_env_conf)
    cfg_file.close()
