...
def backup_zookeeper(db_datas, store_db='local'):
    ...
    with settings(host_string=host):
        ...
        if store_db == 'remote':
            ...
            remote_host = env.roledefs['backup'][0]
            print (msg.format(HOST=remote_host))
        data_path = run("grep  'dataDir'   /etc/zookeeper/conf/zoo.cfg")
...
def reset_mysql_service_token():
    ...
    with settings(host_string=host, warn_only=True):
        mysql_pass = run('cat /etc/contrail/mysql.token')
        ...
        run('find / -name keystone-signing* | xargs -L1 rm -rf')
        run('service supervisor-openstack start')
        time.sleep(10)
        stoken=run('cat /etc/contrail/service.token')
    ...
    with settings(host_string=host):
        if store_db == 'local':
            ...
            if not exists(remote_path):
                ...
                raise SystemExit()
            run('rm -rf %szookeeper' %zoo_path)
            run('rsync -az --progress  %s  %s ' % (remote_path, zoo_path ))
