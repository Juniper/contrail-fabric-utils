            ...
            with settings(host_string=env.roledefs['backup'][0]):
                for bk_path in backup_data_path:
                    ...
                    if exists(snapshot_path):
                        snapshot_dir_path = snapshot_path
    if store_db == 'remote':
        msg = "Restoring backed-up DB and getting  DB dabckup data from remote host:({HOST}) \n"
        remote_host = env.roledefs['backup'][0]
        print (msg.format(HOST=remote_host))
            ...
            with settings(host_string=env.roledefs['backup'][0]):
                for bk_path in backup_data_path:
                    ...
                    if exists(snapshot_path):
                        snapshot_dir_path = snapshot_path
    if store_db == 'remote':
        msg = "Restoring mysql backed-up DB and getting  DB dabckup data from remote host:({HOST}) \n"
        remote_host = env.roledefs['backup'][0]
        print (msg.format(HOST=remote_host))
        remote_path = '%s' % (snapshot_dir_path)
        with settings(host_string=env.roledefs['backup'][0]):
            ...
            with settings(host_string=env.roledefs['backup'][0]):
                for bk_path in backup_data_path:
                    ...
                    if exists('%s/' % snapshot_path):
                        remote_image_path = snapshot_path
    if store_db == 'remote':
        msg = "Restoring backed-up glance image data and getting  backup data from remote host:({HOST}) \n"
        remote_host = env.roledefs['backup'][0]
        print (msg.format(HOST=remote_host))
            ...
            with settings(host_string=env.roledefs['backup'][0]):
                for bk_path in backup_data_path:
                    ...
                    if exists('%sinstances/' % snapshot_path):
                        remote_instances_dir_path = snapshot_path
    if store_db == 'remote':
        msg = "Restoring backed-up instances data and getting  backup data from remote host:({HOST}) \n"
        remote_host = env.roledefs['backup'][0]
        print (msg.format(HOST=remote_host))
