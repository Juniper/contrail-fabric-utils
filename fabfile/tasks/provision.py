...
def add_tsn(reboot= True):
    ...
    execute("add_tsn_node", reboot, env.host_string)

@task
def add_tsn_node(reboot=True,*args):
...
@task
def add_tor_agent_node(reboot=True, *args):
    ...
    for host_string in args:
        with settings(host_string=host_string):
            ...
            for i in range(len(toragent_dict[host_string])):
                ...
                with settings(host_string = '%s@%s' %(cfgm_user, cfgm_ip), password=cfgm_passwd):
                    prov_args = "--host_name %s --host_ip %s --api_server_ip %s --oper add " \
                                "--admin_user %s --admin_password %s --admin_tenant_name %s --openstack_ip %s --router_type tor-agent" \
                                %(agent_name, compute_control_ip, cfgm_ip,
                                  admin_user, admin_password,
                                  admin_tenant_name, keystone_ip)
                    run("python /opt/contrail/utils/provision_vrouter.py %s" %(prov_args))
                    pr_args = "--device_name %s --vendor_name %s --device_mgmt_ip %s --device_tunnel_ip %s --device_tor_agent %s\
                               --device_tsn %s --api_server_ip %s --oper add --admin_user %s\
                               --admin_password %s --admin_tenant_name %s --openstack_ip %s"\
                               %(tor_name, tor_vendor_name, tor_mgmt_ip,tor_tunnel_ip,agent_name,tsn_name,cfgm_ip,
                                admin_user, admin_password,admin_tenant_name, keystone_ip)
