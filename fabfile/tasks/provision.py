    ...
    for host_string in args:
        with  settings(host_string=host_string):
            ...
            if not compute_ceilometer_present:
                sudo("sed -i -e '/\[DEFAULT\]/a %s' /etc/nova/nova.conf" %('notification_driver = ceilometer.compute.nova_notifier'))
                sudo("sed -i -e '/\[DEFAULT\]/a %s' /etc/nova/nova.conf" %('notification_driver = nova.openstack.common.notifier.rpc_notifier'))
                sudo("sed -i -e '/\[DEFAULT\]/a %s' /etc/nova/nova.conf" %('notify_on_state_change = vm_and_task_state'))
                sudo("sed -i -e '/\[DEFAULT\]/a %s' /etc/nova/nova.conf" %('instance_usage_audit_period = hour'))
                sudo("sed -i -e '/\[DEFAULT\]/a %s' /etc/nova/nova.conf" %('instance_usage_audit = True'))
                sudo("service nova-compute restart")

            if host_string != openstack_host:
                #ceilometer.conf updates
                new_line = "rabbit_host=" + amqp_server_ip
                sudo("sed -i -e 's/^#rabbit_host=.*/%s/' /etc/ceilometer/ceilometer.conf" %(new_line))
                new_line = "log_dir=\/var\/log\/ceilometer"
                sudo("sed -i -e 's/^#log_dir=.*/%s/' /etc/ceilometer/ceilometer.conf" %(new_line))
                new_line = "metering_secret=a74ca26452848001921c"
                sudo("sed -i -e 's/^#metering_secret=.*/%s/' /etc/ceilometer/ceilometer.conf" %(new_line))
                new_line = "auth_strategy=keystone"
                sudo("sed -i -e 's/^#auth_strategy=.*/%s/' /etc/ceilometer/ceilometer.conf" %(new_line))
                with settings(warn_only=True):
                    authtoken_config = sudo("grep '^auth_host =' /etc/ceilometer/ceilometer.conf").succeeded
                if not authtoken_config:
                    sudo("sed -i -e '/\[keystone_authtoken\]/a %s' /etc/ceilometer/ceilometer.conf" %('admin_password = CEILOMETER_PASS'))
                    sudo("sed -i -e '/\[keystone_authtoken\]/a %s' /etc/ceilometer/ceilometer.conf" %('admin_user = ceilometer'))
                    sudo("sed -i -e '/\[keystone_authtoken\]/a %s' /etc/ceilometer/ceilometer.conf" %('admin_tenant_name = service'))
                    sudo("sed -i -e '/\[keystone_authtoken\]/a %s' /etc/ceilometer/ceilometer.conf" %('auth_uri = http://'+openstack_ip+':5000'))
                    sudo("sed -i -e '/\[keystone_authtoken\]/a %s' /etc/ceilometer/ceilometer.conf" %('auth_protocol = http'))
                    sudo("sed -i -e '/\[keystone_authtoken\]/a %s' /etc/ceilometer/ceilometer.conf" %('auth_port = 35357'))
                    sudo("sed -i -e '/\[keystone_authtoken\]/a %s' /etc/ceilometer/ceilometer.conf" %('auth_host = ' + openstack_ip))
                    sudo("sed -i -e '/\[service_credentials\]/a %s' /etc/ceilometer/ceilometer.conf" %('os_password = CEILOMETER_PASS'))
                    sudo("sed -i -e '/\[service_credentials\]/a %s' /etc/ceilometer/ceilometer.conf" %('os_tenant_name = service'))
                    sudo("sed -i -e '/\[service_credentials\]/a %s' /etc/ceilometer/ceilometer.conf" %('os_username = ceilometer'))
                    sudo("sed -i -e '/\[service_credentials\]/a %s' /etc/ceilometer/ceilometer.conf" %('os_auth_url = http://'+openstack_ip+':5000/v2.0'))
    ...
    for host_string in args:
        ...
        with  settings(host_string=host_string):
            ...
            if openstack_sku == 'havana':
                ...
                sudo("sed -i -e 's/^connection=.*/%s/' /etc/ceilometer/ceilometer.conf" %(new_line))
                new_line = "rabbit_host=" + amqp_server_ip
                sudo("sed -i -e 's/^#rabbit_host=.*/%s/' /etc/ceilometer/ceilometer.conf" %(new_line))
                new_line = "log_dir=\/var\/log\/ceilometer"
                sudo("sed -i -e 's/^#log_dir=.*/%s/' /etc/ceilometer/ceilometer.conf" %(new_line))
                ...
                if not authtoken_config:
                    ...
                    sudo("sed -i -e '/\[keystone_authtoken\]/a %s' /etc/ceilometer/ceilometer.conf" %('admin_user = ceilometer'))
                    sudo("sed -i -e '/\[keystone_authtoken\]/a %s' /etc/ceilometer/ceilometer.conf" %('admin_tenant_name = service'))
                    sudo("sed -i -e '/\[keystone_authtoken\]/a %s' /etc/ceilometer/ceilometer.conf" %('auth_uri = http://'+self_ip+':5000'))
                    sudo("sed -i -e '/\[keystone_authtoken\]/a %s' /etc/ceilometer/ceilometer.conf" %('auth_protocol = http'))
                    sudo("sed -i -e '/\[keystone_authtoken\]/a %s' /etc/ceilometer/ceilometer.conf" %('auth_port = 35357'))
                    sudo("sed -i -e '/\[keystone_authtoken\]/a %s' /etc/ceilometer/ceilometer.conf" %('auth_host = ' + self_ip))
                    sudo("sed -i -e '/\[service_credentials\]/a %s' /etc/ceilometer/ceilometer.conf" %('os_password = CEILOMETER_PASS'))
                    sudo("sed -i -e '/\[service_credentials\]/a %s' /etc/ceilometer/ceilometer.conf" %('os_tenant_name = service'))
                    sudo("sed -i -e '/\[service_credentials\]/a %s' /etc/ceilometer/ceilometer.conf" %('os_username = ceilometer'))
                    sudo("sed -i -e '/\[service_credentials\]/a %s' /etc/ceilometer/ceilometer.conf" %('os_auth_url = http://'+self_ip+':5000/v2.0'))
...
def setup_image_service_node(*args):
    ...
    for host_string in args:
        ...
        if openstack_sku == 'havana':
            sudo("sed -i -e '/Notification System Options/a %s\n%s\n' /etc/glance/glance-api.conf" \
                       %('notification_driver = messaging', 'rpc_backend = rabbit'))
            new_line = 'notifier_strategy = rabbit'
            sudo("sed -i -e 's/^notifier_strategy =.*/%s/' /etc/glance/glance-api.conf" %(new_line))
            new_line = 'rabbit_host = ' + amqp_server_ip
            sudo("sed -i -e 's/^rabbit_host =.*/%s/' /etc/glance/glance-api.conf" %(new_line))
            sudo("sed -i -e 's/^rabbit_userid =.*/#rabbit_userid = guest/' /etc/glance/glance-api.conf")
            sudo("sed -i -e 's/^rabbit_password =.*/#rabbit_password = guest/' /etc/glance/glance-api.conf")
