...
def get_standby_info(skip_host, match_tor_name):
    ...
    tor_list = get_toragent_nodes()
    for tor in tor_list:
        if tor == skip_host:
            continue
        for i in range(len(toragent_dict[tor])):
            tor_name= toragent_dict[tor][i]['tor_name']
...
def get_all_tor_agent_haproxy_config():
    ...
    for tor in tor_list:
        for i in range(len(toragent_dict[tor])):
            tor_name= toragent_dict[tor][i]['tor_name']
            standby_tor_idx, standby_host = get_standby_info(tor, tor_name)
            if (standby_tor_idx != -1 and standby_host != None):
                tsn1 = toragent_dict[tor][i]['tor_tsn_ip']
                port1 = toragent_dict[tor][i]['tor_ovs_port']
