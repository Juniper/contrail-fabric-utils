    ...
    for host_string in args:
        ...
        with  settings(host_string=host_string):
            ...
            with cd(INSTALLER_DIR):
                ...
                run("PASSWORD=%s ADMIN_TOKEN=%s python setup-vnc-cfgm.py --self_ip %s %s --redis_ip %s --collector_ip %s %s --cassandra_ip_list %s --zookeeper_ip_list %s --quantum_port %s --nworkers %d %s %s %s" %(
                     cfgm_host_password, openstack_admin_password, tgt_ip, keystone_ip, redis_ip,
                     collector_ip, mt_opt, ' '.join(cassandra_ip_list), ' '.join(cassandra_ip_list),
