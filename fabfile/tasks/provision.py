    ...
    for host_string in args:
        ...
        with  settings(host_string=host_string):
            ...
            with cd(INSTALLER_DIR):
                run("PASSWORD=%s ADMIN_TOKEN=%s python setup-vnc-cfgm.py --self_ip %s %s --collector_ip %s %s --cassandra_ip_list %s --zookeeper_ip_list %s --cfgm_index %d --quantum_port %s --nworkers %d --keystone_auth_protocol %s --keystone_auth_port %s --keystone_admin_token %s --keystone_insecure %s %s %s %s" %(
                     cfgm_host_password,
