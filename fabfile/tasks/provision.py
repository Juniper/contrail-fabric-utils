...
def setup_ceilometer_node(*args):
    ...
    for host_string in args:
        # Frame the command line to provision ceilometer in openstack node
        cmd = frame_vnc_ceilometer_cmd(host_string)
        # Execute the provision ceilometer script
        with  settings(host_string=host_string):
            sudo(cmd)
...
def create_toragent_haproxy_node(*args):
    for host_string in args:
        for toragent_host in get_toragent_nodes():
            all_toragent_dict = getattr(env, 'tor_agent')
            for toragent_dict in all_toragent_dict[toragent_host]:
                ip_port_list.append('%s:%s' % toragent_dict[''], toragent_dict[''])
        cmd = 'setup-vnc-toragent-haproxy'
        cmd += ' --tsn_ip_port_list %s' % ' '.join(ip_port_list)
        sudo(cmd)
...
def setup_toragent(manage_nova_compute='yes', configure_nova='yes', restart='yes'):
    ...
    if get_toragent_nodes():
        # Create toragent haproxy config only once
        if get_toragent_nodes().index(env.host_string) == 0:
            execute('create_toragent_haproxy')
        manage_nova_compute = get_manage_nova_compute(manage_nova_compute)
        configure_nova = get_configure_nova(configure_nova)
        # Dont configure nova in vcenter orch or
        # if this node plays tsn role as well,
        if (get_orchestrator() == 'vcenter' or
            env.host_string in get_tsn_nodes()):
            manage_nova_compute='no'
            configure_nova='no'
...
# Backward compatibility for task name
add_tor_agent = setup_toragent

@task
def setup_tor_agent_node(index_range=None, restart='no', *args):
    ...
    for host_string in args:
        with settings(host_string=host_string):
            ...
            if ca_cert_file and os.path.isfile(ca_cert_file):
                put(ca_cert_file, '/etc/contrail/ssl/certs/cacert.pem', use_sudo=True)

            # Frame the command line to provision toragent node for given index range
            # default range is all 
            cmd = frame_vnc_toragent_cmd(host_string, index_range=index_range, restart=restart)
...
@EXECUTE_TASK
@hosts(get_toragent_nodes())
def cleanup_toragent(restart='yes'):
    if get_toragent_nodes():
        execute("cleanup_tor_agent_node",
                None,
                restart,
                env.host_string)

# Backward compatibility for task name
delete_tor_agent = cleanup_toragent

def cleanup_tor_agent_node(index_range=None, restart='no', *args):
    """Disable tor agent functionality for a particular tor in particular node.
        USAGE: fab fab cleanup_tor_agent_node:root@1.1.1.2
    """
    for host_string in args:
        with settings(host_string=host_string):
            # Frame the command line to cleanup toragent node for given index range
            # default range is all 
            cmd = frame_vnc_toragent_cmd(host_string, index_range=index_range,
                                         restart=restart, cmd='cleanup-vnc-tor-agent')
            # Execute the provision toragent script
            sudo(cmd)

@task
def add_tor_agent_by_id(tid, host_string, restart='yes'):
    ...
    for toragent in toragent_dict:
        tor_id = int(toragent['tor_id'])
        if int(tid) == tor_id:
            index = toragent_dict.index(toragent)
        index_range = '%s-%s' % (index, index)
        execute('setup_tor_agent_node',
                index_range, restart, host_string)

@task
def add_tor_agent_by_index_range(range_str, host_string, restart='yes'):
    '''Enable tor agent functionality for a particular tor in particular node.
        USAGE: fab add_tor_agent_by_index_range:0-4,root@1.1.1.2
    '''
    execute('setup_tor_agent_node',
            range_str, restart, host_string)
