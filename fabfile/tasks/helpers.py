...
def preload_image_to_esx(url, glance_id, sizes):
    ...
    for compute_node in env.roledefs['compute']:
        if compute_node in compute_vm_info.keys():
            esx = compute_vm_info[compute_node]
            base = esx['esx_datastore'] + 'vmware_base/'
            with settings(host_string = esx['esxi']['esx_username'] + '@' + esx['esxi']['esx_ip'],
                          password = esx['esxi']['esx_password'], warn_only=True,
                          shell = '/bin/sh -l -c'):
                 with cd(base):
...
@hosts(*env.roledefs['openstack'][0:1])
@task
def add_sanity_images(image=None):
    mount=None
    if '10.84' in env.host:
        mount= '10.84.5.100'
    elif '10.204' in env.host:
    ...
    for (loc, name, sizes) in images:
        ...
        if ".vmdk" in loc:
            ...
            if glance_id.succeeded:
                preload_image_to_esx('http://%s/%s' % (mount,local), glance_id, sizes)
        else:
