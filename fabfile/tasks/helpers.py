...
def reimage_virtual_nodes(host, count):
            with settings(
            host_string='%s@%s' % (
                env.virtual_nodes_info[host]['keystone_user'], env.virtual_nodes_info[host]['keystone_ip']),
                password=env.virtual_nodes_info[host]['keystone_pass'], warn_only=True, abort_on_prompts=False, debug=True):
                i= '--os-username %s --os-password %s --os-tenant-name %s \
                   --os-auth-url http://192.168.22.2:5000/v2.0/' \
                   %(env.virtual_nodes_info[host]['user'], env.virtual_nodes_info[host]['password'],\
                   env.virtual_nodes_info[host]['tenant'])
                check = run('nova %s list | grep %s'%(i, env.hostnames['all'][count]))
                if check:
                   run('nova %s delete %s'%(i, env.hostnames['all'][count]))
                   sleep(5)
                status = run('neutron %s port-create --tenant-id %s --mac-address %s --fixed-ip ip_address=%s --name %s %s' \
                         %(i, env.virtual_nodes_info[host]['tenant_id'], env.virtual_nodes_info[host]['mac-address'], \
                         host.split('@')[1], env.hostnames['all'][count], env.virtual_nodes_info[host]['vn_id']))
                m = re.search('\|\sid(.*)\|\s([\w-]+)(.*)\|',status)
                port_id = m.group(2)
                sleep(10)
                output = run('nova %s boot --flavor 3 --image %s --nic port-id=%s  %s' \
                          %(i, env.virtual_nodes_info[host]['image_name'], port_id, env.hostnames['all'][count] ))
