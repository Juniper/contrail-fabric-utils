...
class Vcenter(object):
    ...
    def add_host(self, cluster, hostname, sslthumbprint, username, password):
        ...
        if host is not None:
            ...
            try:
                ...
                hostspec = vim.host.ConnectSpec(hostName=hostname,sslThumbprint=sslthumbprint,userName=username, password=password)
                ...
                print "Caught vmodl fault : " + error.msg
                return -1
            #host = vim.ClusterComputeResource.AddHost(hostspec)
    ...
    def create_vswitch(self, host_network_system, virt_sw_name, num_ports, nic_name):
        virt_sw_spec = vim.host.VirtualSwitch.Specification()
        virt_sw_spec.numPorts = num_ports
        #vss_spec.bridge = vim.host.VirtualSwitch.SimpleBridge(nicDevice='pnic_key')
        virt_sw_spec.bridge = vim.host.VirtualSwitch.BondBridge(nicDevice=[nic_name])
    ...
    def add_dvPort_group(self, si,dv_switch, dv_port_name):
        dv_pg = self.get_obj([vim.dvs.DistributedVirtualPortgroup], dv_port_name)
    ...
    def create_dvSwitch(self, si, network_folder, cluster, dvs_name):
        ...
        if dvs is not None:
            ...
            return self.get_obj( [vim.DistributedVirtualSwitch],dvs_name)
