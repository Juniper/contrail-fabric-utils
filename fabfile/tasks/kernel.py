...
def upgrade_kernel_all(*tgzs, **kwargs):
    ...
    with settings(host_string=env.roledefs['all'][0], warn_only=True):
        ...
        if version == '12.04':
            ...
            (package, os_type) = ('linux-image-3.13.0-106-generic', 'ubuntu')
            default_grub='Advanced options for Ubuntu>Ubuntu, with Linux 3.13.0-106-generic'
    ...
    for host_string in args:
        with settings(host_string=host_string):
            ...
            if version == '12.04':
                ...
                print "Installing 3.13.0-106 kernel headers"
                apt_install(["linux-headers-3.13.0-106",
                             "linux-headers-3.13.0-106-generic"])
                print "Upgrading the kernel to 3.13.0-106"
                apt_install(["linux-image-3.13.0-106-generic",
...
def migrate_compute_kernel_node(*args):
    for host_string in args:
        with settings(host_string=host_string):
            ...
            sudo('apt-get -o Dpkg::Options::="--force-overwrite" -y install contrail-vrouter-3.13.0-106-generic')
