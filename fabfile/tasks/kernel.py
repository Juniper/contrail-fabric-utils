...
def upgrade_kernel_all(*tgzs, **kwargs):
    ...
    with settings(host_string=env.roledefs['all'][0], warn_only=True):
        ...
        if version == '12.04':
            ...
            (package, os_type) = ('kernel-2.6.32-642.6.2.el6.x86_64', 'centos')
            default_grub='2.6.32-642.6.2.el6.x86_64'
        else:
...
def upgrade_kernel_without_openstack(*tgzs, **kwargs):
    ...
    if 'centos' in dist.lower() and version.startswith('6'):
        (package, os_type) = ('kernel-2.6.32-642.6.2.el6.x86_64', 'centoslinux')
    else:
        raise RuntimeError("Unsupported platfrom (%s, %s, %s) for"
                           " kernel upgrade." % (dist, version, extra))
    ...
    for host_string in args:
        with settings(host_string=host_string):
            ...
            if version == '12.04':
                ...
                print "Upgrading Centos kernel to version 2.6.32-642.6.2"
                pkg_install(["kernel-2.6.32-642.6.2.el6.x86_64",
                             "kernel-headers-2.6.32-642.6.2.el6.x86_64"], disablerepo=False)
                default_grub='2.6.32-642.6.2.el6.x86_64'
                execute('set_grub_default_node', host_string, value=default_grub)
