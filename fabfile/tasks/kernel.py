...
@task
@roles('all')
def set_grub_default(value='Advanced options for Ubuntu>Ubuntu, with Linux 3.13.0-142-generic'):
...
def upgrade_kernel_all(*tgzs, **kwargs):
    ...
    with settings(host_string=env.roledefs['all'][0], warn_only=True):
        ...
        if version == '12.04':
            ...
            (package, os_type) = ('linux-image-3.13.0-142-generic', 'ubuntu')
            default_grub='Advanced options for Ubuntu>Ubuntu, with Linux 3.13.0-142-generic'
        elif 'centos' in dist.lower() and version.startswith('6'):
            (package, os_type) = ('kernel-2.6.32-754.3.5.el6.centos.plus.x86_64', 'centos')
            default_grub='2.6.32-754.3.5.el6.centos.plus.x86_64'
        else:
...
def upgrade_kernel_without_openstack(*tgzs, **kwargs):
    ...
    if 'centos' in dist.lower() and version.startswith('6'):
        (package, os_type) = ('kernel-2.6.32-754.3.5.el6.centos.plus.x86_64', 'centoslinux')
    else:
        raise RuntimeError("Unsupported platfrom (%s, %s, %s) for"
                           " kernel upgrade." % (dist, version, extra))
    ...
    for host_string in args:
        with settings(host_string=host_string):
            ...
            if version == '12.04':
                ...
                print "Installing 3.13.0-142 kernel headers"
                apt_install(["linux-headers-3.13.0-142",
                             "linux-headers-3.13.0-142-generic"])
                print "Upgrading the kernel to 3.13.0-142"
                apt_install(["linux-image-3.13.0-142-generic",
                ...
                print "Upgrading Centos kernel to version 2.6.32-754.3.5.el6.centos.plus.x86_64"
                pkg_install(["kernel-2.6.32-754.3.5.el6.centos.plus.x86_64",
                             "[kernel-headers-2.6.32-754.3.5.el6.centos.plus.x86_64]"], disablerepo=False)
                default_grub='2.6.32-754.3.5.el6.centos.plus.x86_64'
                execute('set_grub_default_node', host_string, value=default_grub)
...
def migrate_compute_kernel_node(*args):
    for host_string in args:
        with settings(host_string=host_string):
            ...
            sudo('apt-get -o Dpkg::Options::="--force-overwrite" -y install contrail-vrouter-3.13.0-142-generic')
