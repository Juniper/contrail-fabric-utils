...
def set_grub_default_node(*args, **kwargs):
    ...
    value = kwargs.get('value', 'Advanced options for Ubuntu>Ubuntu, with Linux 3.13.0-35-generic')
...
@task
@roles('all')
def set_grub_default(value='Advanced options for Ubuntu>Ubuntu, with Linux 3.13.0-35-generic'):
...
def upgrade_kernel_all(reboot='yes'):
    ...
    with settings(host_string=env.roledefs['all'][0], warn_only=True):
        ...
        if version == '12.04':
            ...
            default_grub='Advanced options for Ubuntu>Ubuntu, with Linux 3.13.0-35-generic'
        else:
