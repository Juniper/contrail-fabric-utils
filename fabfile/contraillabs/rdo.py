...
def setup_rdo(rdo_url='https://repos.fedorapeople.org/repos/openstack/openstack-icehouse/rdo-release-icehouse-4.noarch.rpm'):
    ...
    run("openstack-config --set /etc/nova/nova.conf DEFAULT neutron_url http://%s:9696" % testbed.env['roledefs']['cfgm'][0])
    run("openstack-config --set /etc/nova/nova.conf DEFAULT neutron_admin_auth_url http://%s:35357/v2.0" % testbed.env['roledefs']['cfgm'][0])
