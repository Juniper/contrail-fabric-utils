...
class ProvisionNsAgilioVrouter():
    ...
    def check_boot(self):
        print "Check boot"
        run("cd /opt/netronome/srcpkg/; tar -xvf ns-dpdk-srcpkg.tgz > /dev/null")
        run("cd /opt/netronome/srcpkg/ns-dpdk-srcpkg/virtio_relay/; python -m netronome.iommu_check")

        # msr tools
        run("modprobe msr")
        out = run("rdmsr -cx 0x3a -f 2:2")
        if out.strip() != "0x1":
    ...
    def check_netdevs(self):
        ...
        with settings(warn_only=True):
            out = run("ifquery --list | grep -cq nfp_p*")
    ...
    def check_kernel(self):
        ...
        run("/opt/netronome/bin/ns-vrouter-ctl check kernel")


    def check_cpld(self):
        """Check if cpld and cfg rom meet minimums"""

        run("/opt/netronome/bin/ns-vrouter-ctl check cpld")
    ...
    def update_agent_conf(self, name):
        ...
        out = run("openstack-config --set /etc/contrail/contrail-vrouter-agent.conf VIRTUAL-HOST-INTERFACE physical_interface %s" % name)
        out = run("/opt/netronome/bin/ns-vrouter-ctl check mac %s" % name)
        out = run("openstack-config --set /etc/contrail/contrail-vrouter-agent.conf DEFAULT physical_interface_mac %s" % out)
    ...
    def update_agent_param(self, name):
        ...
        run("sed 's/dev=.*/dev=%s/g' /etc/contrail/agent_param > /tmp/agent_param.netro" % name)
        run("mv /tmp/agent_param.netro /etc/contrail/agent_param")
    ...
    def install_openstack_integration(self):
        ...
        run("NS_VROUTER_OPENSTACK_SUPPRESS_CLOUD_ARCHIVE=1 \
    ...
    def configure_virtiorelayd(self):
        ...
        for k,v in self._ns_agilio_vrouter_dict[self._host_string].iteritems():
            if k == 'coremask':
                ...
                run("sed 's/VIRTIORELAYD_CPU_MASK=.*/VIRTIORELAYD_CPU_MASK=%s/g' /etc/default/virtiorelayd > /tmp/virtiorelayd.netro" % v)
                run("mv /tmp/virtiorelayd.netro /etc/default/virtiorelayd")
            elif k == 'pinning_mode':
                print "Setting VIRTIORELAYD_CPU_PINS: (%s, %s)" % (k,v)
                run("sed 's/VIRTIORELAYD_CPU_PINS=.*/VIRTIORELAYD_CPU_PINS=%s/g' /etc/default/virtiorelayd > /tmp/virtiorelayd.netro" % v)
                run("mv /tmp/virtiorelayd.netro /etc/default/virtiorelayd")
            elif k == 'huge_page_alloc':
                print "Setting VIRTIORELAYD_HUGEPAGE_MEMORY_SIZE: (%s, %s)" % (k,v)
                run("sed 's/VIRTIORELAYD_HUGEPAGE_MEMORY_SIZE=.*/VIRTIORELAYD_HUGEPAGE_MEMORY_SIZE=%s/g' /etc/default/virtiorelayd > /tmp/virtiorelayd.netro" % v)
                run("mv /tmp/virtiorelayd.netro /etc/default/virtiorelayd")
            elif k == 'log_level':
                print "Setting VIRTIORELAYD_LOG_LEVEL: (%s, %s)" % (k,v)
                run("sed 's/VIRTIORELAYD_LOG_LEVEL=.*/VIRTIORELAYD_LOG_LEVEL=%s/g' /etc/default/virtiorelayd > /tmp/virtiorelayd.netro" % v)
                run("mv /tmp/virtiorelayd.netro /etc/default/virtiorelayd")
    ...
    def remove_hugepage_grub(self, update_grub=True):
        ...
        run("sed -e 's/\(^GRUB_CMDLINE_LINUX=.*\)\(default_hugepagesz=\w*\s*\)\(.*\)/\\1\\3/g' -e 's/\(^GRUB_CMDLINE_LINUX=.*\)\(hugepagesz=\w*\s*\)\(.*\)/\\1\\3/g' -e 's/\(^GRUB_CMDLINE_LINUX=.*\)\(hugepages=\w*\s*\)\(.*\)/\\1\\3/g' /etc/default/grub > /tmp/grub.netro")
        run("mv /tmp/grub.netro /etc/default/grub")

        print "Removing hugepage config from GRUB_CMDLINE_LINUX_DEFAULT"
        run("sed -e 's/\(^GRUB_CMDLINE_LINUX_DEFAULT=.*\)\(default_hugepagesz=\w*\s*\)\(.*\)/\\1\\3/g' -e 's/\(^GRUB_CMDLINE_LINUX_DEFAULT=.*\)\(hugepagesz=\w*\s*\)\(.*\)/\\1\\3/g' -e 's/\(^GRUB_CMDLINE_LINUX_DEFAULT=.*\)\(hugepages=\w*\s*\)\(.*\)/\\1\\3/g' /etc/default/grub > /tmp/grub.netro")
        run("mv /tmp/grub.netro /etc/default/grub")

        # remove trailing whitespace (will touch comment lines)
        run("sed -e 's/\s*\"$/\"/g' /etc/default/grub > /tmp/grub.netro")
        run("mv /tmp/grub.netro /etc/default/grub")
    ...
    def add_hugepage_grub(self, update_grub=True):
        ...
        run("sed -e 's/\(^GRUB_CMDLINE_LINUX_DEFAULT=.*\)\"$/\\1 %s\"/' /etc/default/grub > /tmp/grub.netro" % grub_cmdline_add)
        run("mv /tmp/grub.netro /etc/default/grub")
    ...
    def disable_virtiorelayd(self):
        ...
        # modify conf file
        run("sed 's/VIRTIORELAYD_ENABLE=.*/VIRTIORELAYD_ENABLE=false/g' /etc/default/virtiorelayd > /tmp/virtiorelayd.netro")
        run("mv /tmp/virtiorelayd.netro /etc/default/virtiorelayd")

    def enable_virtiorelayd(self):
        print "Enable virtiorelayd"
        # modify conf file
        run("sed 's/VIRTIORELAYD_ENABLE=.*/VIRTIORELAYD_ENABLE=true/g' /etc/default/virtiorelayd > /tmp/virtiorelayd.netro")
        run("mv /tmp/virtiorelayd.netro /etc/default/virtiorelayd")
    ...
    def update_interfaces_file(self, name):
        ...
        # download tempfile
        get(remote_path="/etc/network/interfaces", local_path=iface_tmp_path)
        ...
        local("cat %s" % iface_tmp_path) 
        # upload tempfile
        put(local_path=iface_tmp_path, remote_path="/etc/network/interfaces")
