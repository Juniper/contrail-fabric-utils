...
def vcenter_server_set_defaults(server):
    # If datacenter mtu is not configured, set the default.
    for datacenter in server['datacenters']:
        if not 'datacenter_mtu' in datacenter.keys():
            datacenter['datacenter_mtu'] = 1500
    return
